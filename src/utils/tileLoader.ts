...
export class TileLoader {
  ...
  async loadTiles(): Promise<Tile[]> {
    ...
    try {
      ...
      if (!metaResponse.ok) {
        throw new Error(`Failed to fetch tiles_meta.json: ${metaResponse.status}`)
      }
      
      this.tilesData = await metaResponse.json() as TileAtlasData
      console.log('Loaded metadata:', this.tilesData.tiles.length, 'tiles found')
  ...
  private categorizeTile(spriteName: string): string {
    ...
    if (spriteName.includes('door2') || spriteName.includes('door3') || spriteName.includes('door4')) return 'passages'
    // gates (excluding switch_iron_gate) stay in passages  
    if (spriteName.includes('gate') && !spriteName.includes('switch_iron_gate')) return 'passages'
    // switches and buttons (excluding switch_wall which is handled above)
    if (spriteName.includes('switch') || spriteName.includes('button')) return 'switches'
    if (spriteName.includes('statue') || spriteName.includes('treasure')) return 'objects'
    ...
    if (spriteName.includes('door') || spriteName.includes('pitfall') || spriteName.includes('floor') || spriteName.includes('wall')) return 'connections'
    if (spriteName.includes('water') || spriteName.includes('current')) return 'water'
    if (spriteName.includes('enemy') || spriteName.includes('golem')) return 'enemies'
  ...
  isEdgeTile(tile: Tile): boolean {
    ...
    return name.includes('switch_wall') || 
           name.includes('door2') || 
           name.includes('door3') || 
           name.includes('door4')
  }
  ...
  drawTileForPalette(ctx: CanvasRenderingContext2D, tile: Tile, maxSize: number = 38): void {
    ...
    ctx.drawImage(
      this.tilesImage,
      tile.textureRect.x, tile.textureRect.y,
  ...
  extractTileSprite(tile: Tile, size: number = 40): HTMLCanvasElement {
    ...
    if (this.tilesImage) {
      ...
      ctx.drawImage(
        this.tilesImage,
        tile.textureRect.x, tile.textureRect.y,
        tile.textureRect.width, tile.textureRect.height,
        0, 0, size, size
      )
    }
  ...
  drawTileWithMaxSize(ctx: CanvasRenderingContext2D, tile: Tile, x: number, y: number): void {
    if (this.tilesImage) {
      try {
        const originalWidth = tile.textureRect.width
        const originalHeight = tile.textureRect.height
        ...
        if (placementType === 'edge') {
          ...
          } else if (subX === 1 && subY === 2) { // Bottom edge
            drawX = logicalCellX + (120 - drawWidth) / 2 // Center horizontally in 120px
            drawY = logicalCellY + 120 - drawHeight / 2 // Center on bottom edge line
          } else if (subX === 0 && subY === 1) { // Left edge
        ...
        const needsRotation = placementType === 'edge' && 
  ...
  drawTileAtOriginalSize(ctx: CanvasRenderingContext2D, tile: Tile, x: number, y: number): void {
    if (this.tilesImage) {
      try {
        ctx.drawImage(
          this.tilesImage,
          tile.textureRect.x, tile.textureRect.y,
          tile.textureRect.width, tile.textureRect.height,
          x, y, tile.textureRect.width, tile.textureRect.height
        )
      } catch (error) {
  ...
  drawTile(ctx: CanvasRenderingContext2D, tile: Tile, x: number, y: number, size: number = 40): void {
    if (this.tilesImage) {
