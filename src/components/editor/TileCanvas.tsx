...
const TileCanvas = ({
  ...
  const scaleFactor =
    maxDimension > maxCanvasSize ? maxCanvasSize / maxDimension : 1;
  ...
  const getAccessiblePlacementType = (
    tilePos: MapPosition
  ): 'edge' | 'normal' | 'invalid' => {
    ...
    const isDoorTile =
      selectedTile?.id.toLowerCase().includes('door') &&
  ...
  const getExpandedPlacementType = (
    tilePos: MapPosition
  ): 'edge' | 'normal' | 'invalid' => {
    const subX = tilePos.x % 3;
    const subY = tilePos.y % 3;
    console.log(
      'getExpandedPlacementType called with tilePos:',
      tilePos,
      'subX:',
      subX,
      'subY:',
    ...
    const isDoorTile =
      selectedTile?.id.toLowerCase().includes('door') &&
      !selectedTile?.id.toLowerCase().includes('gate_switch');
    const isEdgeTile = selectedTile
      ? tileLoader.isEdgeTile(selectedTile)
      : false;
  ...
  const isPositionOnPath = (tilePos: MapPosition): boolean => {
    ...
    return paths.some(
      pathPoint => pathPoint.x === logicalX && pathPoint.y === logicalY
    );
  };
  ...
  const addPathPoint = (
    ...
    if (
      tilePos.x < 0 ||
      tilePos.x >= mapData.width ||
      tilePos.y < 0 ||
      tilePos.y >= mapData.height
    ) {
      return;
    }
  ...
  const placeTile = (tilePos: MapPosition) => {
    ...
    if (placementType === 'edge' && !isEdgeTile) {
      console.log(
        'Only edge tiles (doors/switches) can be placed on edge positions'
      );
      return;
    }
  ...
  const drawPathWalls = (ctx: CanvasRenderingContext2D) => {
    ...
    paths.forEach(pathPoint => {
      ...
      if (!hasTop) {
        ctx.fillRect(cellX, cellY, cellSize, wallWidth);
      }

      // Right wall - positioned at right edge
      if (!hasRight) {
        ctx.fillRect(cellX + cellSize - wallWidth, cellY, wallWidth, cellSize);
      }

      // Bottom wall - positioned at bottom edge
      if (!hasBottom) {
        ctx.fillRect(cellX, cellY + cellSize - wallWidth, cellSize, wallWidth);
      }

      // Left wall - positioned at left edge
      if (!hasLeft) {
        ctx.fillRect(cellX, cellY, wallWidth, cellSize);
      }
    });
  };
    ...
    arrows.forEach((arrow, arrowIndex) => {
      ...
      for (let i = 0; i < arrow.points.length; i++) {
        ...
        if (overlappingArrows.length > 0 && i > 0) {
          ...
          const perpOffset = calculatePerpendicularOffset(
            prevPoint,
            currentPoint,
            offsetDistance
          );
  ...
  const drawArrows = (ctx: CanvasRenderingContext2D) => {
    const commentLayer = mapData.layers.find(
      layer => layer.type === 'comments'
    );
    if (
      appMode !== 'comment' ||
      !commentLayer ||
      !commentLayer.visible ||
      !commentLayer.arrows
    )
      return;
    ...
    const drawArrowPath = (
      ...
      const startX =
        firstPoint.x * mapData.tileSize + mapData.tileSize / 2 + firstOffset.x;
      const startY =
        firstPoint.y * mapData.tileSize + mapData.tileSize / 2 + firstOffset.y;
      ...
      for (let i = 1; i < points.length; i++) {
        ...
        const endX =
          point.x * mapData.tileSize + mapData.tileSize / 2 + offset.x;
        const endY =
          point.y * mapData.tileSize + mapData.tileSize / 2 + offset.y;
      ...
      const endX =
        lastPoint.x * mapData.tileSize + mapData.tileSize / 2 + lastOffset.x;
      const endY =
        lastPoint.y * mapData.tileSize + mapData.tileSize / 2 + lastOffset.y;
      const prevX =
        secondLastPoint.x * mapData.tileSize +
        mapData.tileSize / 2 +
        secondLastOffset.x;
      const prevY =
        secondLastPoint.y * mapData.tileSize +
        mapData.tileSize / 2 +
        secondLastOffset.y;
    ...
    if (isDrawingArrow && currentArrowPoints.length > 0 && mousePosition) {
      ...
      const previewPoints = currentArrowPoints.map(point =>
        getTileCenterPosition(point)
      );
      ...
      if (!isValidSegment && currentArrowPoints.length > 0) {
        ...
        const lastPoint = getTileCenterPosition(
          currentArrowPoints[currentArrowPoints.length - 1]
        );
    ...
    if (tileLoader.getTileImage()) {
      try {
        ...
        ctx.drawImage(
          ...
          selectedTile.textureRect.x,
          selectedTile.textureRect.y,
          originalWidth,
          originalHeight,
          -drawWidth / 2,
          -drawHeight / 2,
  ...
  const isStraightLine = (
    ...
    const isHorizontal =
      center1.y === center2.y && Math.abs(center1.x - center2.x) > 0;
    const isVertical =
      center1.x === center2.x && Math.abs(center1.y - center2.y) > 0;

    return isHorizontal || isVertical;
  };

  // Check if line segment passes through walls
  const doesLinePassThroughWalls = (
    point1: MapPosition,
    point2: MapPosition
  ): boolean => {
    const center1 = getTileCenterPosition(point1);
  ...
  const arePointsAdjacentAndAligned = (
    point1: MapPosition,
    point2: MapPosition
  ): boolean => {
    const center1 = getTileCenterPosition(point1);
  ...
  const createArrowFromPoints = (points: MapPosition[]) => {
    ...
    const commentLayerIndex = updatedMapData.layers.findIndex(
      layer => layer.type === 'comments'
    );
    if (commentLayerIndex >= 0) {
      updatedMapData.layers[commentLayerIndex] = {
        ...updatedMapData.layers[commentLayerIndex],
        arrows: [
          ...(updatedMapData.layers[commentLayerIndex].arrows || []),
          newArrow,
        ],
      };
    }
  ...
  const removeArrowAtPosition = (tilePos: MapPosition) => {
    ...
    const arrowsToRemove = commentLayer.arrows.filter(arrow => {
      return arrow.points.some(point => {
        const distance =
          Math.abs(point.x - clickedCenter.x) +
          Math.abs(point.y - clickedCenter.y);
        return distance <= 1; // Remove if click is within 1 tile of any point
      });
    });

    if (arrowsToRemove.length > 0) {
      const remainingArrows = commentLayer.arrows.filter(
        arrow => !arrowsToRemove.some(toRemove => toRemove.id === arrow.id)
      );
  ...
  const switchWaterTileVariant = async (direction: 1 | -1) => {
    ...
    const loadedTiles = await tileLoader.loadTiles();
    const waterTiles = loadedTiles.filter(tile =>
      tile.id.includes('water_current')
    );

    if (waterTiles.length === 0) return;

    // Find current tile index
    const currentIndex = waterTiles.findIndex(
    ...
    const nextIndex =
      (currentIndex + direction + waterTiles.length) % waterTiles.length;
  ...
  return (
    <div className="overflow-auto">
      ...
      <div className="mt-2 text-sm text-muted-foreground">
        Canvas size: {canvasWidth} × {canvasHeight}px | Grid: {mapData.width} ×{' '}
        {mapData.height} tiles | Tile size: {mapData.tileSize}px
      </div>
    </div>
  );
};
